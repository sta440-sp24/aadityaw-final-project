...
The primary outcome of this study is a set of segregation statistics to spatially describe caste/tribe-based segregation in the MMR at a shrid-level. These will comprise of the aspatial global dissimilarity index, Reardon and O'Sullivan's set of surface-based spatial dissimilarity indices [@Reardon_2004], and Morrill's areal-based, spatially-adjusted dissimilarity index [@Morrill_1992], comparing SC and ST population percentages vs. all other individuals as well as firm ownership for SC, ST, and OBC populations vs. all others. Thes statistics are theoretically bounded between 0 and 1, with 1 being some measure of perfect segregation/no diversity and 0 being no segregation/perfect diversity (elaborated upon in Results).
...
prim |>
  rownames_to_column() |>
  kable(digits = 3, col.names = c("Comparison Type", "Aspatial D Index", "Morrill's D Index",
...
This project's analysis showed less segregation against SC individuals than initially may have been expected, but still displayed convincing evidence 


Many of the limitations of this study arose from data availability and quality. Ideally, this study would have been conducted at the Ward Level, which would haven given us granularity for specific neighborhoods in the region that have sociological and historical value. 

It would also have been conducted with newer data

The fact that the city proper only achieved the granularity of two shrids may have affeted our 

Methodologically, we must also acknowledge that many of these measures are, relatively speaking, new in the literature and each suffers from limitations unique to them. tHE S

Tehc 

Finally, we must rbitrary measure of firm ownership 
