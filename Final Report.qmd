...
ggplot() + 
  ...
  mutate(sc_st_prop = (pc11_pca_p_sc + pc11_pca_p_st) / 
                                  pc11_pca_tot_p),
  aes(fill = sc_st_prop), color = ifelse(mmr_shrid$shrid2 == "11-27-518-99999-802794", "red", NA)) +
...
These metrics were calculated using the *seg* package in R, developed in part by Sullivan. As the *seg* package has been temporarily unavailable on CRAN since Feb 2024, owing to some dependency issues, an older version of the package available on the archive was used instead; the tar.gz can be found in the project repo.
