...
x <- as_Spatial(st_geometry(mmr_shrid_2))


df <- tibble(sc_pop = mmr_shrid_2$pc11_pca_p_sc +  mmr_shrid_2$pc11_pca_p_st, other_pop = mmr_shrid_2$pc11_pca_tot_p - (sc_pop))

# spseg(x, df, smoothing = "kernel", sigma = 0.03)

dissim(x, df)


# try dissim with larger units? - possible now, data fixed
# fix districts for mumbai - done
# try econ census data - done
